...
const UpdateOrder = ({ orderData }) => {
  ...
  useEffect(() => {
    if (orderData) {
      ...
      setStatus(orderData.status || 'In Production');
      setExpectedDelivery(orderData.expected_delivery || '');
      setLoading(false);
    } else {
      // If no order data is provided, we could either:
      // 1. Show an error
      // 2. Redirect back
      // 3. Initialize with empty values
      console.error("No order data provided for update");
      setLoading(false);
    }
  }, [orderData]);
  ...
  const handleSubmit = (e) => {
    ...
    // updateOrder(updatedData).then(() => dispatch(closePoop()));
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            ...
            <input
              ...
              readOnly // Order ID typically shouldn't be changed after creation
            />
          </div>
